...
const Dashboard = () => {
  ...
  const handleDeposit = async (vaultName: string, apy: string) => {
    ...
    try {
      ...
      setVyrrResponse("Authorization failed. The transaction was aborted. I'm standing by for your next instruction.");
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="glass-card p-8 md:p-10 rounded-3xl">
          <div className="flex items-center justify-between mb-10">
            <h3 className="text-2xl font-black italic text-white uppercase tracking-tight">Available Strategies</h3>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {vaults.map((vault) => (
                <div key={vault.id} className="flex flex-col md:flex-row items-center justify-between p-6 glass-card border-white/5 rounded-2xl hover:bg-white/10 transition-all group gap-8">
                  <div className="flex items-center gap-6 w-full md:w-auto">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xl font-black text-white italic uppercase tracking-tight">{vault.name}</span>
                        <ExternalLink size={14} className="text-slate-600" />
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Risk: {vault.risk}</span>
                        <div className="h-1 w-1 rounded-full bg-slate-700" />
                        <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">{vault.project}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-10 w-full md:w-auto">
                    <div className="text-center md:text-right">
                      <span className="block font-black text-pink-500 text-3xl italic tracking-tighter text-glow">{vault.apy}</span>
